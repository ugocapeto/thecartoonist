...
void run_oabf(int pass, FImage *sourceLab, FImage *target, FImage *tfm, float sigma_d, float sigma_r)
{
  ...
  for (int y = 0; y < h; y++) 
  {
    ...
    {        
      ...
      //use the get() function that takes in int instead of float
      tfm->get(x, y, tangent);
      float t[3];
      if (pass == 0) {
        t[0]= tangent[1];
  ...
  float sigma_e, float tau)
{
  ...
  {    
    for (int x = 0; x < w; x++) 
    {
      ...
      uv[2] = 0.f;
      float t[3];
      //tfm->get(constrain((int)roundf(uv[0]), 0, w-1), constrain((int)roundf(uv[1]), 0, h-1), t);
      //use the get() function that takes in int instead of floa
      tfm->get(x, y, t);
      float n[3];
...
void step(FImage *tfm, lic_t *s) 
{
  ...
  //because get() linearly interpolates and you could have t[0]=0 and t[1]=0
  //also, it's probably a good idea to normalize t
  float m = sqrtf(t[0]*t[0]+t[1]*t[1]);
  if (m > 0.f)
  {
     t[0] /= m;
     t[1] /= m;
  ...
  {    
    ...
    {        
      ...
      {       
        ...
        //I am adding some code here
        //because get() linearly interpolates and you could have v1[0]=0 and v1[1]=0
        //also, it's probably a good idea to normalize v1
        float m = sqrtf(v1[0]*v1[0]+v1[1]*v1[1]);
        if (m > 0.f)
        {
           v1[0] /= m;
           v1[1] /= m;
      ...
      //I am not gonna use lambda to make the strongest lines redder
